-- Canonical, idempotent schema + seed for the restaurant site.
-- Safe to run repeatedly and on a fresh project: every object uses an
-- IF (NOT) EXISTS / OR REPLACE / DROP-then-CREATE guard, and all seed
-- INSERTs use ON CONFLICT DO NOTHING against natural-key unique indexes.
-- Supersedes the two duplicate 2025-08-05 migrations.
...
-- ---------------------------------------------------------------------------
-- Natural-key unique indexes (enable idempotent ON CONFLICT seeding)
-- ---------------------------------------------------------------------------
CREATE UNIQUE INDEX IF NOT EXISTS opening_hours_day_key      ON public.opening_hours (day_of_week);
CREATE UNIQUE INDEX IF NOT EXISTS menu_categories_name_key   ON public.menu_categories (name);
CREATE UNIQUE INDEX IF NOT EXISTS drink_categories_name_key  ON public.drink_categories (name);
CREATE UNIQUE INDEX IF NOT EXISTS menu_items_cat_name_key    ON public.menu_items (category_id, name);
CREATE UNIQUE INDEX IF NOT EXISTS drink_items_cat_name_key   ON public.drink_items (category_id, name);
CREATE UNIQUE INDEX IF NOT EXISTS contact_info_type_label_key ON public.contact_info (type, label);

-- ---------------------------------------------------------------------------
-- Row Level Security: public read-only (restaurant content is public)
-- ---------------------------------------------------------------------------
...
DROP POLICY IF EXISTS "Drink categories are publicly readable" ON public.drink_categories;
DROP POLICY IF EXISTS "Drink items are publicly readable"    ON public.drink_items;
...
DROP TRIGGER IF EXISTS update_opening_hours_updated_at    ON public.opening_hours;
DROP TRIGGER IF EXISTS update_menu_categories_updated_at  ON public.menu_categories;
DROP TRIGGER IF EXISTS update_menu_items_updated_at       ON public.menu_items;
DROP TRIGGER IF EXISTS update_drink_categories_updated_at ON public.drink_categories;
DROP TRIGGER IF EXISTS update_drink_items_updated_at      ON public.drink_items;
...
-- ---------------------------------------------------------------------------
-- Seed data (idempotent: ON CONFLICT DO NOTHING on natural keys)
-- ---------------------------------------------------------------------------
